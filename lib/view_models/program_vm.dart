...
class ProgramViewModel extends ChangeNotifier with DialogMixin {
  ...
  Future<void> fetchAllPrograms() async {
    ...
    if(currLatitude==null&&currLongitude==null){
      ...
      resp = await _net.request('/programs?latawal=${currLatitude}&longawal=${currLongitude}', queryParameter: {});
    }
    if (resp != null) {
      ...
      for (dynamic data in listData)
        program_list.add(Program.fromJson(data));
      isLoading = false;
    } else {
  ...
  Future<Iterable<dynamic>> getProgramRecommendations(keyword) async {
    ...
    print('Koordinat: ${currLatitude}');
    if(currLatitude==null&&currLongitude==null){
      resp = await _net.request('/programs?keyword=${keyword}', queryParameter: {});
    } else {
      resp = await _net.request('/programs?latawal=${currLatitude}&longawal=${currLongitude}&keyword=${keyword}', queryParameter: {});
    }
  ...
  Future<void> fetchRoutes() async {
    ...
    PolylineResult result = await polylinePoints.getRouteBetweenCoordinates(
      ...
      PointLatLng(double.parse(latakhir_route!), double.parse(lonakhir_route!)),
      travelMode: TravelMode.driving,
      avoidTolls: jenis_kendaraan == 'MOTORCYCLE' ? true : false
    );

    if(result.points.length==0){
      showErrorSnackbar('Maaf rute tidak tersedia untuk mode transportasi yang dipilih');
    } else {
  ...
  Future<void> fetchDirections() async {
      isLoading = true;
      var fullUrl = '${GMAP_API_URL}directions/json?origin=${latawal_route}%2C${lonawal_route}&destination=${latakhir_route}%2C${lonakhir_route}&language=id&key=${GMAP_API_KEY}';
      var resp;
      resp = await await http.get(
        Uri.parse(fullUrl),
      );
      ...
      if(response_decoded['status']=='OK'){
        ...
        if(response_decoded.length>0){
          ...
          for(var instruction in steps){
            if(i==0){
              route_instructions.add(new Instruction(desc: instruction['html_instructions'], duration: instruction['duration']['text'], icon: Icons.straight, distance: instruction['distance']['text']));
            } else {
              route_instructions.add(new Instruction(desc: instruction['html_instructions'], duration: instruction['duration']['text'], icon: detectIcon(instruction['maneuver']), distance: instruction['distance']['text']));
            }
            i++;
          }
        }
  ...
  Future<void> fetchAllBanks() async {
    ...
    if (resp != null) {
      ...
      List<dynamic> allMethods = resp.data['data'];

      for (dynamic data in allMethods) all_bank.add(PaymentMethod.fromJson(data));
      isLoading = false;
    }
    notifyListeners();
  }
